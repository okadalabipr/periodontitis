...
class Visualization(Observable):
    ...
    def get_single_observable_options(self):
        ...
        #    col=j/50.0
        #    cmapcal=[col,col,0.0]
        #    cmaplist.append(cmapcal)
        #    shapelist.append('D')
        
        for i, _ in enumerate(self.obs_names):
            self.single_observable_options[i].divided_by = 3  # sec. -> min.
            self.single_observable_options[i].xlim = (0, 17)
            self.single_observable_options[i].xticks = [0, 4, 8, 12, 16]
            self.single_observable_options[i].xlabel = 'Time (day)'
            self.single_observable_options[i].ylabel = 'Activity AU (AU)'
            self.single_observable_options[i].ylim = (0, 1.6)
            #self.single_observable_options[i].yticks = [0 , 20 ,40,60,80,100]
            
            self.single_observable_options[i].cmap = cmaplist
            self.single_observable_options[i].shape = shapelist
    ...
    def get_multiple_observables_options(self):
        ...
        self.multiple_observables_options.condition = 'EGF20'
